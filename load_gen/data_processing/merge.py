...
input_directory = working_dir+"loadgen"
...
def merge_csvs(cassandra, netdata):
    # Leitura dos arquivos CSV
    df1 = pd.read_csv(cassandra)
    df2 = pd.read_csv(netdata)
    ...
    merged_df.to_csv(working_dir+'/write.csv', index=False)
...
def join_netdata_cassandra():
    ...
    for local in locais:
        ...
        if final_df.empty:
            ...
            final_df = pd.concat(
                [final_df, df_cpu.drop(columns='time'), df_mem.drop(columns='time'), df_ipv4.drop(columns='time')],
                axis=1)

    # Exibir o DataFrame final
    print(final_df.head())

    # Converter a coluna 'time' para tipo datetime
    final_df['time'] = pd.to_datetime(final_df['time'])

    # Aplicar um shift de três horas
    final_df['time'] = final_df['time'] + pd.Timedelta(hours=-3)

    # Converter para Unix timestamp (em segundos)
    final_df['time'] = final_df['time'].astype(int) // 10 ** 9
    # Salvar o DataFrame final em um arquivo CSV, se necessário
    final_df.to_csv('netdata.csv', index=False)
